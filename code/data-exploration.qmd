...
seasonal_summary <- bird_weather_monthly |> 
  ...
  # create season variable from month
  mutate(
    season = case_when(
  ...
  # convert abundance and precipitation into one column for faceting
  pivot_longer(
  ...
  # rename variables for clearer facet labels
  mutate(
    variable = case_when(
...
ggplot(data = seasonal_summary,
  ...
  # add points at each season
  geom_point(size = 3) +
  
  # separate abundance and precipitation into different panels
  facet_wrap(~ variable,
           scales = "free_y",
           ncol = 1) +
